...
var loopProtect = (function () {
	...
	var loopProtect = {
		...
		method: 'window.runnerWindow.protect',
	};
	...
	loopProtect.rewriteLoops = function (code, offset) {
		...
		function renderProtect(lineNumber, protect) {
			var str = loopProtect.method + "(" + lineNumber;
			if (protect)
				str += ",true";
			return str + ")";
		}
		...
		var insertReset = function (lineNum, line, matchPosition) {
			...
			// outside the logic
			return line.slice(0, matchPosition) + ';' + renderProtect(lineNum, true) + ";" + line.slice(matchPosition);
		};
				...
				while (index < line.length) {
					...
					if (openBrackets === 0 && (character === ';' || character === '{')) {
						...
						if (lineNum === originalLineNum && labelPostion === null) {
							...
							index += (';' + renderProtect(printLineNumber, true) + '; ').length;
						} else {
	 ...
	 */
	loopProtect.protect = function (lineNumber, reset) {
		var now = Date.now();
		var line = loopProtect.counters[lineNumber];
		if (!line)
			line = loopProtect.counters[lineNumber] = { time: now };
		else if (reset)
			line.time = now;
		else if ((now - line.time) > 100) {
			// We've spent over 100ms on this loop... smells infinite.
			var msg = 'Exiting potential infinite loop at line ' + lineNumber + '. To disable loop protection: add "// noprotect" to your code';
			(window.proxyConsole || console).error(msg);
