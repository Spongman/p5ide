...
declare class p5 {
  ...
  /**
   * Draw an arc to the screen. If called with only a, b, c, d, start, and stop, the arc will be drawn as an open pie. If mode is provided, the arc will be drawn either open, as a chord, or as a pie as specified. The origin may be changed with the ellipseMode() function.
   * 
   ...
   * 
   *  The fifth, sixth, seventh and eighth parameters, if specified, determine corner radius for the top-right, top-left, lower-right and lower-left corners, respectively. An omitted corner radius parameter is set to the value of the previously specified radius value in the parameter list.
   */
   ...
   * 
   *  While print() is similar to console.log(), it does not directly map to it in order to simulate easier to understand behavior than console.log(). Due to this, it is slower. For fastest results, use console.log().
   */
  ...
  /**
   * The deviceMoved() function is called when the device is moved by more than the threshold value along X, Y or Z axis. The default threshold is set to 0.5.
   */
  deviceMoved(): void
   ...
   * 
   * var d = pixelDensity; for (var i = 0; i < d; i++) { for (var j = 0; j < d; j++) { // loop over idx = 4 * ((y * d + j) * width * d + (x * d + i)); pixels[idx] = r; pixels[idx+1] = g; pixels[idx+2] = b; pixels[idx+3] = a; } }  
   * 
   ...
   * 
   *  Available blend modes are: BLEND | DARKEST | LIGHTEST | DIFFERENCE | MULTIPLY| EXCLUSION | SCREEN | REPLACE | OVERLAY | HARD_LIGHT | SOFT_LIGHT | DODGE | BURN | ADD | NORMAL
   */
   ...
   * 
   *  For more advanced use, you may also pass in the path as the first argument and a object as the second argument, the signature follows the one specified in the Fetch API specification.
   */
   ...
   * 
   *  save(); save('myCanvas.jpg'); // save a specific canvas with a filename  
   * 
  ...
  /**
   * Normal material for geometry. You can view all possible materials in this example.
   */
  normalMaterial(): p5
  ...
  /**
   * Specular material for geometry with a given color. You can view all possible materials in this example.
   */
  ...
  /**
   * Searches the page for an element with the given ID, class, or tag name (using the '#' or '.' prefixes to specify an ID or class respectively, and none for a tag) and returns it as a p5.Element. If a class or tag name is given with more than 1 element, only the first element will be returned. The DOM node itself can be accessed with .elt. Returns null if none found. You can also specify a container to search within.
   */
  select(name: string, container?: string): any|p5.Element|null
  
  /**
   * Searches the page for elements with the given class or tag name (using the '.' prefix to specify a class and no prefix for a tag) and returns them as p5.Elements in an array. The DOM node itself can be accessed with .elt. Returns an empty array if none found. You can also specify a container to search within.
   */
  ...
  /**
   * Creates an <a></a> element in the DOM for including a hyperlink. Appends to the container node if one is specified, otherwise appends to body.
   */
  createA(href: string, html: string, target?: string): any|p5.Element
  
  /**
   * Creates a slider <input></input> element in the DOM. Use .size() to set the display length of the slider. Appends to the container node if one is specified, otherwise appends to body.
   */
  createSlider(min: number, max: number, value?: number, step?: number): any|p5.Element
  
  /**
   * Creates a <button></button> element in the DOM. Use .size() to set the display size of the button. Use .mousePressed() to specify behavior on press. Appends to the container node if one is specified, otherwise appends to body.
   */
  ...
  /**
   * Creates a radio button <input></input> element in the DOM. The .option() method can be used to set options for the radio after it is created. The .value() method will return the currently selected option.
   */
  createRadio(divId?: string): any|p5.Element
  
  /**
   * Creates an <input></input> element in the DOM for text input. Use .size() to set the display length of the box. Appends to the container node if one is specified, otherwise appends to body.
   */
...
declare namespace p5 {
  ...
  class Color {
    /**
     * We define colors to be immutable objects. Each color stores the color mode and level maxes that applied at the time of its construction. These are used to interpret the input arguments and to format the output e.g. when saturation() is requested. 
     * 
  ...
  class Element {
    /**
     * Base class for all elements added to a sketch, including canvas, graphics buffers, and other HTML elements. Methods in blue are included in the core functionality, methods in brown are added with the p5.dom library. It is not called directly, but p5.Element objects are created by calling createCanvas, createGraphics, or in the p5.dom library, createDiv, createImg, createInput, etc.
     */
    ...
    /**
     * The .mouseReleased() function is called once after every time a mouse button is released over the element. This can be used to attach element specific event listeners.
     */
    mouseReleased(fxn: () => any): p5.Element
    
    /**
     * The .mouseClicked() function is called once after a mouse button is pressed and released over the element. This can be used to attach element specific event listeners.
     */
    ...
    /**
     * The .changed() function is called when the value of an element is changed. This can be used to attach an element specific event listener.
     */
    ...
    /**
     * Sets the position of the element relative to (0, 0) of the window. Essentially, sets position:absolute and left and top properties of style. If no arguments given returns the x and y position of the element in an object.
     */
    ...
    /**
     * Sets the width and height of the element. AUTO can be used to only adjust one dimension. If no arguments given returns the width and height of the element in an object.
     */
  ...
  class Renderer extends p5.Element {
    /**
     * Main graphics and rendering context, as well as the base API implementation for p5.js "core". To be used as the superclass for Renderer2D and Renderer3D classes, respecitvely.
     */
  ...
  class Image {
     ...
     * 
     * If no params are passed, those whole image is returned, if x and y are the only params passed a single pixel is extracted if all params are passed a rectangle region is extracted and a p5.Image is returned. 
     * 
    ...
    /**
     * Masks part of an image from displaying by loading another image and using it's alpha channel as an alpha channel for this image.
     */
    ...
    /**
     * Saves the image to a file and force the browser to download it. Accepts two strings for filename and file extension Supports png (default) and jpg.
     */
  ...
  class Font {
    ...
    /**
     * Returns a tight bounding box for the given text string using this font (currently only supports single lines)
     */
  ...
  class MediaElement {
    ...
    /**
     * Send the audio output of this element to a specified audioNode or p5.sound object. If no element is provided, connects to p5's master output. That connection is established when this method is first called. All connections are removed by the .disconnect() method. 
     * 
  ...
  class SoundFile {
    ...
    /**
     * Multiply the output volume (amplitude) of a sound file between 0.0 (silence) and 1.0 (full volume). 1.0 is the maximum amplitude of a digital sound, so multiplying by greater than 1.0 may cause digital distortion. To fade, provide a rampTime parameter. For more complex fades, see the Env class. 
     * 
    ...
    /**
     * Return the current position of the p5.SoundFile playhead, in seconds. Note that if you change the playbackRate while the p5.SoundFile is playing, the results may not be accurate.
     */
    currentTime(): number
  ...
  class Oscillator {
     ...
     * 
     * Set the type of oscillation with setType(), or by creating a specific oscillator. For example: new p5.SinOsc(freq) new p5.TriOsc(freq) new p5.SqrOsc(freq) new p5.SawOsc(freq).
     */
    constructor(freq?: number, type?: string)
    
    /**
     * Start an oscillator. Accepts an optional parameter to determine how long (in seconds from now) until the oscillator starts.
     */
  ...
  class Env {
    ...
    /**
     * Play tells the envelope to start acting on a given input. If the input is a p5.sound object (i.e. AudioIn, Oscillator, SoundFile), then Env will control its output volume. Envelopes can also be used to control any  Web Audio Audio Param.
     */
    ...
    /**
     * Multiply the p5.Env's output amplitude by a fixed value. Calling this method again will override the initial mult() with new values.
     */
  ...
  class AudioIn {
    ...
    /**
     * Client must allow browser to access their microphone / audioin source. Default: false. Will become true when the client enables acces.
     */
    enabled: boolean
    ...
    /**
     * Chrome only. Returns a list of available input sources and allows the user to set the media source. Firefox allows the user to choose from input sources in the permissions dialogue instead of enumerating available sources and selecting one. Note: in order to have descriptive media names your page must be served over a secure (HTTPS) connection and the page should request user media before enumerating devices. Otherwise device ID will be a long device ID number and does not specify device type. For example see https://simpl.info/getusermedia/sources/index.html vs. http://simpl.info/getusermedia/sources/index.html
     */
    getSources(callback: () => any): void
    
    /**
     * Set the input source. Accepts a number representing a position in the array returned by listSources(). This is only available in browsers that support MediaStreamTrack.getSources(). Instead, some browsers give users the option to set their own media source.
     */
  ...
  class PeakDetect {
     ...
     * 
     *  Based on example contribtued by @b2renger, and a simple beat detection explanation by a href="http://www.airtightinteractive.com/2013/10/making-audio-reactive-visuals/" target="_blank"Felix Turner.
     */
