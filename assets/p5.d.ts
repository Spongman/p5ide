...
declare class p5 {
   ...
   * 
   *  
   *   Note that if only one value is provided to 
   ...
   *   array. 
   * 
   *   Hue exists in both HSB and HSL. This function will 
   *   return the HSB-normalized hue when supplied with 
   *   an HSB color object (or when supplied with a pixel 
   *   array while the color mode is HSB), but will 
   *   default to the HSL-normalized hue otherwise. (The 
   *   values will only be different if the maximum hue 
   *   setting for each system is different.)
   */
   ...
   * 
   *  
   *   The way that colours are interpolated depends on 
   ...
   *   This function will return the HSB saturation when 
   *   supplied with an HSB color object (or when 
   *   supplied with a pixel array while the color mode 
   *   is HSB), but will default to the HSL saturation 
   *   otherwise.
   */
   ...
   * 
   *  
   *   The color is either specified in terms of the RGB, 
   *   HSB, or HSL color depending on the current 
   *   colorMode. (The default color space is RGB, with 
   ...
   * 
   *  
   *   Note: existing color objects remember the mode 
   *   that they were created in, so you can change modes 
   *   as you like without affecting their appearance.
   */
   ...
   * 
   *  
   *   Note that drawing a full circle (ex: 0 to TWO_PI) 
   *   will appear blank because 0 and TWO_PI are the 
   *   same position on the unit circle. The best way to 
   *   handle this is by using the ellipse() function 
   *   instead to create a closed ellipse, and to use the 
   *   arc() function only to draw parts of an ellipse.
   */
   ...
   * 
   *  
   *   The fifth, sixth, seventh and eighth parameters, 
   *   if specified, determine corner radius for the 
   ...
   *   given to ellipse() are interpreted. 
   * 
   *  
   *   The default mode is ellipseMode(CENTER), which 
   *   interprets the first two parameters of ellipse() 
   *   as the shape's center point, while the third and 
   ...
   *   drawn by changing the way in which parameters 
   *   given to rect() are interpreted. 
   * 
   *  
   *   The default mode is rectMode(CORNER), which 
   *   interprets the first two parameters of rect() as 
   *   the upper-left corner of the shape, while the 
   ...
   * 
   *  
   *   It should always be controlled with noLoop(), 
   *   redraw() and loop(). After noLoop() stops the code 
   *   in draw() from executing, redraw() causes the code 
   *   inside draw() to execute once, and loop() will 
   *   cause the code inside draw() to resume executing 
   *   continuously. 
   * 
   *  
   ...
   *   them.
   * 
   *   Bezier curves were developed by French automotive 
   *   engineer Pierre Bezier, and are commonly used in 
   *   computer graphics to define gently sloping curves. 
   *   See also curve().
   */
   ...
   *   describe the other control point. 
   * 
   *  
   *   Longer curves can be created by putting a series 
   *   of curve() functions together or using 
   *   curveVertex(). An additional function called 
   *   curveTightness() provides control for the visual 
   *   quality of the curve. The curve() function is an 
   ...
   * 
   *  
   *   While print() is similar to console.log(), it does 
   *   not directly map to it in order to simulate easier 
   *   to understand behavior than console.log(). Due to 
   *   this, it is slower. For fastest results, use 
   *   console.log().
   */
   ...
   * 
   *  
   *   Calling frameRate() with no arguments returns the 
   *   current framerate. The draw function must run at 
   *   least once before it will return a value. This is 
   ...
   * 
   *  
   *   When noLoop() is used, it's not possible to 
   *   manipulate or access the screen inside event 
   *   handling functions such as mousePressed() or 
   ...
   *   transformation state and style settings controlled 
   *   by the following functions: fill(), stroke(), 
   *   tint(), strokeWeight(), strokeCap(), strokeJoin(), 
   *   imageMode(), rectMode(), ellipseMode(), 
   *   colorMode(), textAlign(), textFont(), textMode(), 
   ...
   * 
   *  
   *   In structuring a program, it only makes sense to 
   *   call redraw() within events such as 
   ...
   *   objects in a clockwise direction. Transformations 
   *   apply to everything that happens after and 
   *   subsequent calls to the function accumulates the 
   *   effect. For example, calling rotate(HALF_PI) and 
   *   then rotate(HALF_PI) is the same as rotate(PI). 
   *   All tranformations are reset when draw() begins 
   *   again. 
   * 
   *  
   ...
   *   draw(), the transformation is reset when the loop 
   *   begins again. 
   * 
   *  
   ...
   * 
   *  
   *   Transformations are cumulative and apply to 
   ...
   *   translate(20, 0) is the same as translate(70, 0). 
   *   If translate() is called within draw(), the 
   *   transformation is reset when the loop begins 
   *   again. This function can be further controlled by 
   *   using push() and pop().
   */
   ...
   *   beginShape()/endShape() pair and transformations 
   *   such as translate(), rotate(), and scale() do not 
   *   work within a beginContour()/endContour() pair. It 
   *   is also not possible to use other shapes, such as 
   ...
   *   beginShape() call, it must be prefaced with a call 
   *   to vertex() to set the first anchor point. This 
   *   function must be used between beginShape() and 
   ...
   * 
   *  
   *   The first and last points in a series of 
   *   curveVertex() lines will be used to guide the 
   *   beginning and end of a the curve. A minimum of 
   *   four points is required to draw a tiny curve 
   *   between the second and third points. Adding a 
   *   fifth point with curveVertex() will draw the curve 
   ...
   * 
   *  
   *   Unlike rotationX and rotationY, this variable is 
   *   available for devices with a built-in compass 
   *   only. 
   * 
   *  
   *   Note: The order the rotations are called is 
   ...
   * 
   *  
   *   pRotationX can also be used with rotationX to 
   *   determine the rotate direction of the device along 
   *   the X-axis.
   */
  pRotationX: number
   ...
   * 
   *  
   *   pRotationY can also be used with rotationY to 
   *   determine the rotate direction of the device along 
   *   the Y-axis.
   */
  pRotationY: number
   ...
   * 
   *  
   *   pRotationZ can also be used with rotationZ to 
   *   determine the rotate direction of the device along 
   *   the Z-axis.
   */
  pRotationZ: number
   ...
   * 
   *  
   *   The axis that triggers the deviceTurned() method 
   *   is stored in the turnAxis variable. The 
   *   deviceTurned() method can be locked to trigger on 
   *   any axis: X, Y or Z by comparing the turnAxis 
   ...
   * 
   *  
   *   For non-ASCII keys, use the keyCode variable. You 
   *   can check if the keyCode equals BACKSPACE, DELETE, 
   *   ENTER, RETURN, TAB, ESCAPE, SHIFT, CONTROL, 
   *   OPTION, ALT, UP_ARROW, DOWN_ARROW, LEFT_ARROW, 
   *   RIGHT_ARROW. 
   * 
   *  
   ...
   * 
   *  
   *   The event.delta property returns the amount the 
   *   mouse wheel have scrolled. The values can be 
   *   positive or negative depending on the scroll 
   *   direction (on OS X with "natural" scrolling 
   ...
   *   id properties. 
   * 
   *   The touches[] array is not supported on Safari and 
   *   IE on touch-based desktops (laptops).
   */
  touches: any[]
   ...
   *  
   *   Browsers may have different default behaviors 
   *   attached to various touch events. To prevent any 
   *   default behavior for this event, add "return 
   *   false" to the end of the method.
   */
  touchStarted(): void
   ...
   * 
   *  
   *   .pixels gives access to an array containing the 
   *   values for all the pixels in the display window. 
   *   These values are numbers. This array is the size 
   *   (including an appropriate factor for the 
   *   pixelDensity) of the display window x4, 
   *   representing the R, G, B, A values in order for 
   *   each pixel, moving from left to right across each 
   *   row, then down each column. See .pixels for more 
   *   info. It may also be simpler to use set() or 
   *   get(). 
   * 
   *  
   ...
   * 
   *  
   *   The image may not be immediately available for 
   *   rendering If you want to ensure that the image is 
   *   ready before doing anything with it, place the 
   *   loadImage() call in preload(). You may also supply 
   *   a callback function to handle the image when it's 
   *   ready. 
   * 
   *  
   ...
   *   transparent by including an alpha value. 
   * 
   *  
   *   To apply transparency to an image without 
   *   affecting its color, use white as the tint color 
   *   and specify an alpha value. For instance, 
   *   tint(255, 128) will make an image 50% transparent 
   *   (assuming the default alpha range of 0-255, which 
   *   can be changed with colorMode()). 
   * 
   *  
   ...
   * 
   *  
   *   The first four values (indices 0-3) in the array 
   *   will be the R, G, B, A values of the pixel at (0, 
   *   0). The second four values (indices 4-7) will 
   *   contain the R, G, B, A values of the pixel at (1, 
   *   0). More generally, to set values for a pixel at 
   *   (x, y): 
   * 
   ...
   * 
   *  
   *   Available blend modes are: BLEND | DARKEST | 
   *   LIGHTEST | DIFFERENCE | MULTIPLY| EXCLUSION | 
   *   SCREEN | REPLACE | OVERLAY | HARD_LIGHT | 
   ...
   * 
   *  
   *   If the pixel requested is outside of the image 
   ...
   *   directly to the display window. 
   * 
   *   The x and y parameters specify the pixel to change 
   *   and the c parameter specifies the color value. 
   *   This can be a p5.Color object, or [R, G, B, A] 
   *   pixel array. It can also be a single grayscale 
   *   value. When setting an image, the x and y 
   ...
   *   here.
   */
  loadJSON(path: string, jsonpOptions?: any, datatype?: string, callback?: () => any, errorCallback?: () => any): any|any[]
   ...
   * 
   *  
   *   Alternatively, the file maybe be loaded from 
   *   anywhere on the local computer using an absolute 
   *   path (something that starts with / on Unix and 
   *   Linux, or a drive letter on Windows), or the 
   *   filename parameter can be a URL for a file found 
   *   on a network. 
   * 
   *  
   ...
   */
  loadTable(filename: string, options?: string, callback?: () => any, errorCallback?: () => any): any
   ...
   */
  httpGet(path: string, datatype?: string, data?: any, callback?: () => any, errorCallback?: () => any): void
   ...
   */
  httpPost(path: string, datatype?: string, data?: any, callback?: () => any, errorCallback?: () => any): void
   ...
   * 
   *  
   *   For more advanced use, you may also pass in the 
   *   path as the first argument and a object as the 
   *   second argument, the signature follows the one 
   *   specified in the Fetch API specification.
   */
  httpDo(path: string, method?: string, datatype?: string, data?: any, callback?: () => any, errorCallback?: () => any): void
   ...
   *   specified in the Fetch API specification.
   */
  httpDo(path: string, options: any, callback?: () => any, errorCallback?: () => any): void
   ...
   *   open a new save dialog every frame. 
   * 
   *   The default behavior is to save the canvas as an 
   *   image. You can optionally specify a filename. For 
   *   example: 
   * 
   *  
   ...
   */
  save(objectOrFilename?: any|string, filename?: string, options?: boolean|string): void
   ...
   * 
   *  
   *   In the first example above, the number 25 is 
   ...
   * 
   *  
   *   By default, noise is computed over 4 octaves with 
   *   each octave contributing exactly half than its 
   *   predecessor, starting at 50% strength for the 1st 
   *   octave. This falloff amount can be changed by 
   *   adding an additional function parameter. Eg. a 
   ...
   *   pseudo-random numbers each time the software is 
   *   run.
   */
   ...
   * 
   *  
   *   Note: The y-coordinate of the point is the first 
   *   parameter, and the x-coordinate is the second 
   *   parameter, due the the structure of calculating 
   *   the tangent.
   */
   ...
   *   BASELINE). 
   * 
   *   The horizAlign parameter is in reference to the x 
   *   value of the text() function, while the vertAlign 
   *   parameter is in reference to the y value. 
   * 
   ...
   */
  loadFont(path: string, callback?: () => any): p5.Font
   ...
   *   function, which gives the option to draw to the 
   *   left, right, and center of the coordinates. 
   * 
   *  
   ...
   * 
   *  
   *   The simplified version with only two arguments, 
   *   arrayCopy(src, dst), copies an entire array to 
   *   another of the same size. It is equivalent to 
   ...
   * 
   *  
   *   To use the function, first check to see if the 
   *   result is null. If the result is null, then the 
   *   sequence did not match at all. If the sequence did 
   ...
   *   second dimension is only of length one. 
   * 
   *  
   *   To use the function, first check to see if the 
   *   result is null. If the result is null, then the 
   ...
   *   pieces. 
   * 
   *   The splitTokens() function works in a similar 
   *   fashion, except that it splits using a range of 
   *   characters instead of a specific character or 
   *   sequence.
   */
   ...
   * 
   *  
   *   If no delim characters are specified, any 
   *   whitespace character is used to split. Whitespace 
   ...
   * 
   *  
   *   One of the limitations of the OBJ format is that 
   ...
   */
  select(name: string, container?: string): any|p5.Element|null
   ...
   */
  createImg(src: string, alt?: string, successCallback?: () => any): any|p5.Element
   ...
   *   body.
   */
  createA(href: string, html: string, target?: string): any|p5.Element
   ...
   */
  createSlider(min: number, max: number, value?: number, step?: number): any|p5.Element
   ...
   *   otherwise appends to body.
   */
  createButton(label: string, value?: string): any|p5.Element
   ...
   */
  createCheckbox(label?: string, value?: boolean): any|p5.Element
   ...
   *   select box
   */
  createSelect(existing: any): p5.Element
   ...
   *   option.
   */
  createRadio(divId?: string): any|p5.Element
   ...
   */
  createInput(value?: number, type?: string): any|p5.Element
   ...
   */
  createFileInput(callback?: () => any, multiple?: string): any|p5.Element
   ...
   *   onto the canvas using video(). 
   * 
   *   More specific properties of the feed can be 
   *   passing in a Constraints object. See the  W3C spec 
   *   for possible properties. Note that not all of 
   ...
   *   here and here.
   */
  createCapture(type: string|any|any, callback: () => any): any|p5.Element
...
declare namespace p5 {
  ...
  class Color {
     ...
     *   requested. 
     * 
     *   Internally we store an array representing the 
     *   ideal RGBA values in floating point form, 
     *   normalized from 0 to 1. From this we calculate the 
     *   closest screen color (RGBA levels from 0 to 255) 
     *   and expose this to the renderer. 
     * 
  ...
  class Element {
     ...
     */
    doubleClicked(fxn: () => any): p5.Element
     ...
     * 
     *  
     *   The function accepts a callback function as 
     *   argument which will be executed when the wheel 
     ...
     *   dropped.
     */
    drop(callback: () => any, fxn: () => any): p5.Element
     ...
     */
    addClass(theClass: string): any|p5.Element
    
    /**
     *   Removes specified class from the element.
     */
    removeClass(theClass: string): any|p5.Element
     ...
     *   element.
     */
    html(html?: string, append?: boolean): any|p5.Element|string
     ...
     */
    position(x?: number, y?: number): any|p5.Element
     ...
     *   10, 100, 50);
     */
    style(property: string, value?: string|number|p5.Color): string|any|p5.Element
     ...
     */
    attribute(attr: string, value?: string): string|any|p5.Element
    
    /**
     *   Removes an attribute on the specified element.
     */
    removeAttribute(attr: string): any|p5.Element
    
    /**
     *   Either returns the value of the element if no 
     *   arguments given, or sets the value of the element.
     */
    value(value?: string|number): string|any|p5.Element
     ...
     */
    size(w?: number, h?: number): any|p5.Element
  ...
  class Image {
     ...
     *   backed representation of an image. 
     * 
     *  
     *   p5 can display .gif, .jpg and .png images. Images 
     *   may be displayed in 2D and 3D space. Before an 
     *   image is used, it must be loaded with the 
     *   loadImage() function. The p5.Image class contains 
     *   fields for the width and height of the image, as 
     *   well as an array called pixels[] that contains the 
     ...
     * 
     *   If no params are passed, those whole image is 
     *   returned, if x and y are the only params passed a 
     *   single pixel is extracted if all params are passed 
     *   a rectangle region is extracted and a p5.Image is 
     *   returned. 
     * 
     ...
     *   be slower than directly manipulating the pixels 
     *   array and then calling updatePixels().
     */
  ...
  class Table {
     ...
     *   using set(). 
     * 
     *   If a p5.TableRow object is included as a 
     *   parameter, then that row is duplicated and added 
     *   to the table.
     */
     ...
     *   "tokens"). 
     * 
     *   If no column is specified, then the values in all 
     *   columns and rows are processed. A specific column 
  ...
  class TableRow {
     ...
     *   values, stored in columns, from a table. 
     * 
     *   A Table Row contains both an ordered array, and an 
  ...
  class Vector {
     ...
     * 
     *  
     *   In many of the p5.js examples, you will see 
     *   p5.Vector used to describe a position, velocity, 
     *   or acceleration. For example, if you consider a 
     *   rectangle moving across the screen, at any given 
     *   instant it has a position (a vector that points 
     *   from the origin to its location), a velocity (the 
     *   rate at which the object's position changes per 
     *   time unit, expressed as a vector), and 
  ...
  class Geometry {
     ...
     *   p5 Geometry class
     */
    constructor(vertData: () => any|any, detailX?: number, detailY?: number, callback?: () => any)
  }
  ...
  class MediaElement {
     ...
     */
    noLoop(): any|p5.Element
    
    /**
     *   Set HTML5 media element to autoplay or not.
     */
    autoplay(autoplay: boolean): any|p5.Element
     ...
     *   might not be smooth.)
     */
    speed(speed?: number): number|any|p5.MediaElement
     ...
     */
    time(time?: number): number|any|p5.MediaElement
     ...
     */
    onended(callback: () => any): any|p5.MediaElement
     ...
     *   removed by the .disconnect() method. 
     * 
     ...
     *   point. 
     * 
     *   Accepts a callback function, a time (in seconds) 
     *   at which to trigger the callback, and an optional 
  ...
  class SoundFile {
     ...
     * 
     *   The p5.SoundFile may not be available immediately 
     ...
     */
    constructor(path: string|any[], successCallback?: () => any, errorCallback?: () => any, whileLoadingCallback?: () => any)
     ...
     */
    loadSound(path: string|any[], successCallback?: () => any, errorCallback?: () => any, whileLoading?: () => any): SoundFile
     ...
     *   class. 
     * 
     *   Alternately, you can pass in a signal source such 
     *   as an oscillator to modulate the amplitude with an 
     *   audio signal.
     */
     ...
     *   it thinks there is a beat. 
     * 
     *   This is an asynchronous function that processes 
     *   the soundfile in an offline audio context, and 
     *   sends the results to your callback function. 
     * 
  ...
  class FFT {
     ...
     *   frequencies within a waveform. 
     * 
     *   Once instantiated, a p5.FFT object can return an 
     *   array based on two types of analyses: 
     *  
  ...
  class Oscillator {
     ...
     *   (440Hz, equal to the pitch of an 'A' note). 
     * 
     *   Set the type of oscillation with setType(), or by 
    ...
    // TODO: Fix amp() errors in lib/addons/p5.sound.js, line 4563:
    //
    //   return has invalid type: AudioParam
    //
    // amp(vol: number|any, rampTime?: number, timeFromNow?: number): any
    
    // TODO: Fix freq() errors in lib/addons/p5.sound.js, line 4598:
    //
    //   return has invalid type: AudioParam
    //
  ...
  class Env {
     ...
     *   Use setADSR to change attackTime, decayTime, 
     *   sustainPercent and releaseTime. 
     * 
  ...
  class AudioIn {
     ...
     *   microphone. 
     * 
     *   Turn the mic on/off with the start() and stop() 
     *   methods. When the mic is on, its volume can be 
     *   measured with getLevel or by connecting an FFT 
     *   object. 
     * 
  ...
  class Effect {
    /**
     *   Effect is bases class for audio effects in p5 This 
     *   module handles the nodes and methods that are 
     *   common and useful for all current and future 
     *   effects.
     */
    constructor()
  ...
  class Convolver extends p5.Effect {
     ...
     *   called  convolution. 
     * 
     *   Convolution multiplies any audio input by an 
     *   "impulse response" to simulate the dispersion of 
     *   sound over time. The impulse response is generated 
     *   from an audio file that you provide. One way to 
     *   generate an impulse response is to pop a balloon 
     *   in a reverberant space and record the echo. 
     *   Convolution can also be used to experiment with 
     *   sound. 
     * 
     ...
     *   position in the .impulses  Array (Number).
     *  
     *   You can access the objects in the .impulses Array 
  ...
  class Phrase {
     ...
     *   i.e. a series of notes and rests. 
     * 
     *   Phrases must be added to a p5.Part for playback, 
     *   and each part can play multiple phrases at the 
     *   same time. For example, one Phrase might be a kick 
     *   drum, another could be a snare, and another could 
     *   be the bassline. 
     * 
  ...
  class Compressor extends p5.Effect {
     ...
     *   especially useful when many sounds are being used 
     *   at once. Compression can be used on indivudal 
     *   sound sources in addition to the master output. 
     *   This class is built using the Web Audio Dynamics 
     *   Compressor Node 
    ...
    /**
     *   set threshold w/ time ramp or get current 
     *   threshold
     */
    threshold(): void
  ...
  class SoundRecorder {
     ...
     *   specific source with setInput(). 
     * 
     *   The record() method accepts a p5.SoundFile as a 
     *   parameter. When playback is stopped (either after 
     *   the given amount of time, or with the stop() 
     *   method), the p5.SoundRecorder will send its 
  ...
  class PeakDetect {
     ...
     * 
     *   Accepts an FFT object. You must call .analyze() on 
     *   the FFT object prior to updating the peakDetect 
  ...
  class Distortion extends p5.Effect {
     ...
     *   distortion.
     */
    setType(amount?: number, oversample?: string): void
